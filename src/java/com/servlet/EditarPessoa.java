    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           //Criando uma instancia para a morada
            Morada morada = new Morada(request.getParameter("bairro").trim(),
            ...
            if(new MoradaDao().insert(morada)){
                ...
                int idPessoa = pDao.findOne(pessoa);
                if(idPessoa>0){
                ...
                //response.sendRedirect("Paginas/pessoa.jsp?erro="+ERRO);
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CadastroPessoa at SARAAAAAAAAAAAAAA " + request.getContextPath() + "</h1>");
            out.println("<h2>"+new MoradaDao().findOne(morada)+"</h2>");
            out.println("<h2>"+pessoa.toString()+"</h2>");
            out.println("<h3>"+request.getParameter("dataNasc").trim()+"</h3>");
            out.println("</body>");
            out.println("</html>");
