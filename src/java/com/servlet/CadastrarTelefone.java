...
public class CadastrarTelefone extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(verificarCampos(request)){
                ...
                if(pId>0){
                    ...
                    if(new TelefoneDao().insert(telefone)){
                        mensagem = "Telefone de "+nome[0]+" "+ nome[1]+", cadastrado com sucesso!!";
                    }
                    else
    ...
    public boolean verificarCampos(HttpServletRequest request){
        if(request.getParameter("telefone").trim() != null && request.getParameter("pessoa").trim()!=null)
            return true;
        return false;
    }
